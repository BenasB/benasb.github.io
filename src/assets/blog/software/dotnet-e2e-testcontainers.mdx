...
Passing the `AppFixture` for the `IClassFixture<T>`, xUnit will inject the `AppFixture` into our test class constructor. The fixture at this point is already initialzed (`InitializedAsync` was called) and our containers should be running. We can see that all we need in the test from the fixture are the two clients to communicate with the APIs. The test code itself is pretty straighforward and readable so it's nice that we contained all of the Testcontainers setup behind the scenes in the fixture.
...
- Notifier is a .NET worker service which, upon getting a message from the Blog Events queue, retrieves the blog post title from Blog DB, retrieves all subscribers from Newsletter DB and sends them an email informing about the new blog post.
...
We can also visualize how the test should look like from a network perspective again:

![Network of example 2](vector/e2e-example2-network.svg)

We can move onto the code. The approach is a bit different this time: every container will have a separate fixture and the `AppFixture` will aggregate them.
...
Like mentioned before, `AppFixture` aggregates all of these container fixtures.
...
Again, we can see that the test code itself is pretty straighforward and all of the Testcontainers setup is left to the `AppFixture` which allows you to not mix your test logic with the setup.
